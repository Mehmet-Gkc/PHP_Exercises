...
// GET Kurs bei ID
    ...
    function getCoursesByKeyword($q) {
        ...
        $query = "SELECT * from kurse WHERE title LIKE '%$q%' or altBaslik LIKE '%$q%'";
        $sonuc = mysqli_query($baglanti,$query);
        mysqli_close($baglanti);
        return $sonuc;
    };
...
// CREATE Kurs
    ...
    function createCourse(string $title, string $unterTitle, string $beschreibung ,string $img,int $kategorie_id, int $komment=0, int $begeni=0, int $urkunde=0) {
    include "connection.php";

    $query = "INSERT INTO kurse(title,unterTitle,beschreibung,img,kategorie_id,komment,begeni,urkunde) VALUES (?,?,?,?,?,?,?,?)";
    $stmt = mysqli_prepare($baglanti,$query);

    mysqli_stmt_bind_param($stmt, 'ssssiiii', $title, $unterTitle, $beschreibung,$img, $kategorie_id, $komment, $begeni, $urkunde);
    mysqli_stmt_execute($stmt);
    mysqli_stmt_close($stmt);
...
// EDIT Kurs
    function editCourse(int $id, string $title, string $unterTitle, string $beschreibung, string $img,int $kategorie_id, int $urkunde) {
    include "connection.php";

    $query = "UPDATE kurse SET title='$title', unterTitle='$unterTitle',beschreibung='$beschreibung' ,img='$img',kategorie_id='$kategorie_id', urkunde='$urkunde' WHERE id=$id";
    $sonuc = mysqli_query($baglanti,$query);
    mysqli_close($baglanti);
    return $sonuc;
};
